...
export async function createProject(formData: FormData) {
  ...
  const raw = String(formData.get("default_country") ?? "").trim().toUpperCase();
  const defaultCountry = COUNTRIES.some((c) => c.code === raw) ? raw : "IN";

  const res = await fetch(`${BACKEND_URL}/api/projects`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name, domain, default_country: defaultCountry }),
  });
  if (!res.ok) {
